...
public class ArmorHUD extends HudElement {
    ...
    private final Setting<Boolean> blur = this.sgGeneral.booleanSetting("Blur Effect", true, "Applies a blur effect.");
    ...
    @Override
    public void render() {
        boolean hasPlayer = BlackOut.mc.player != null;
        boolean mainMenu = !hasPlayer;

        if (mainMenu) {
            float bgWidth = 4.0F + (4 * 16.0F) + (3 * 6.0F);
            float bgHeight = 18.0F;
            this.setSize(bgWidth, bgHeight);

            this.stack.pushPose();
            this.drawPlaceholder(this.stack);
            this.stack.popPose();
            return;
        }
    ...
    private void drawPlaceholder(PoseStack stack) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            Render2DUtils.renderItem(stack, PLACEHOLDER_ARMOR[this.reversed.get() ? i : 3 - i], xOffset, 1.0F, 16.0F, RenderLayer.HUD, false);
        }
        BlackOut.mc.renderBuffers().bufferSource().endBatch();
    }
