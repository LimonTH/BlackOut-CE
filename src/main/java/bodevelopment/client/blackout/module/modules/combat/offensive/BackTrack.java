...
public class BackTrack extends Module {
    ...
    public final TickTimerList<Pair<RemotePlayer, AABB>> hit = new TickTimerList<>(false);
    public final TickTimerList<Pair<RemotePlayer, AABB>> spoofed = new TickTimerList<>(false);
    public final TimerMap<RemotePlayer, Vec3> realPositions = new TimerMap<>(false);
    ...
    public void onSend(PacketEvent.Send event) {
        if (event.packet instanceof ServerboundInteractPacket packet) {
            ...
            if (accessor.getType().getType() == ServerboundInteractPacket.ActionType.ATTACK && BlackOut.mc.level.getEntity(accessor.getId()) instanceof RemotePlayer player) {
                ...
                this.hit.remove(timer -> timer.value.getA().equals(player));
                this.hit.add(pair, this.time.get());
                if (!this.spoofed.contains(timer -> timer.value.getA().equals(player))) {
                    this.spoofed.remove(timer -> timer.value.getA().equals(player));
                        ...
                        (key, value) -> {
                            if (System.currentTimeMillis() > value.endTime) {
                                ...
                                AABB box = new AABB(
                                        ...
                                        value.value.z() - 0.3,
                                        value.value.x() + 0.3,
                                        value.value.y() + key.getBoundingBox().getYsize(),
                                        value.value.z() + 0.3
                                );
    ...
    public void onTick(TickEvent.Pre event) {
        this.hit.timers.removeIf(item -> {
            if (item.ticks-- <= 0) {
                this.spoofed.remove(timer -> ((Pair) item.value).getA().equals(timer.value.getA()));
                return true;
            } else {
                return false;
            }
